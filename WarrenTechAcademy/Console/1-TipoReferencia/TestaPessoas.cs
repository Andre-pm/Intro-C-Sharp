...
namespace Console._1_TipoReferencia
{
    internal class TestaPessoas
    {
        ...
        public static void ExecutaJuridica()
        {
            ...
            System.Console.WriteLine(repository.Create(pessoaJuridica2));

            List<PessoaJuridica> juridicas = repository.Read();
        ...
        {
            Animal animal = new Animal(nome: "Capivara", nomeCientifico: "Hydrochoerus hydrochaeris", peso: 50, categoria: "Herbívoro");
            AnimalRepository repo = new AnimalRepository();

            System.Console.WriteLine(repo.Create(animal));

            List<Animal> animais = repo.Read();

            foreach (Animal ani in animais)
            {
